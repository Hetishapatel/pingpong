...
/*
 * Date: 12/08/2021
 * Authors:  Hetisha Patel, Pratik Bhogale, Karan Gonagur
 * Task: Multi user Pong game with single joystick control, left and right walls represnt the paddle area while top and bottom endges are the walls the ball should always bounce off
...
//        UARTprintf("js position: %d\n", getYCoordinate(ui32ADC0Value[1], 0, 3800));
    ...
    while(1)
    {
        if(GPIOPinRead(GPIO_PORTD_BASE, GPIO_PIN_6) == 0 || score > 9)
        {
            //if the reset button is pressed or the score reaches 10, restart the game
            score = 0;
            SysCtlDelay(3000000);
            ST7735_FillScreen(0xFFFF);
...
//        UARTprintf("xCoor: %d    yCoor: %d\n", xCoor, yCoor);
        ...
        {
            // increase score/loss count
            ++score;
            scorec = score + '0';
            SysCtlDelay(300000);
            ST7735_FillScreen(0xFFFF);
            ST7735_DrawCharS (60, 60, scorec, ST7735_Color565(255, 0, 0), 0, 3);
            SysCtlDelay(9000000);
            createBallToStart(&xi, &yi);
            initializeBallStartParams(&direction, &index, &dx, &dy);
...
void initialisePortsAndGpios()
{
    ...
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOF);            // Enable GPIO for buzzer
...
//int xr = 118, yr = 66, xb = 61, yb = 122, xl = 5, yl = 66, xt = 61, yt = 14;
    ...
    if(ballXCoor > 119 && (ballYCoor <= paddleYCoor || (ballYCoor >= paddleYCoor - 16)))
    {
        // if balls passes the left wall/paddle area
        return 0;
    }
...
//        *dx = (-1) * (*dx);
//    }
    if(((nextY <= (paddleYCoor)  && nextY > paddleYCoor - 16) && (nextX >= 119)) || ((ballYCoor <= (paddleYCoor)  && ballYCoor > paddleYCoor - 16) && (ballXCoor >= 119)))
    {
        buzz(); // make the buzzer sound when hits the paddle
        //bouncing off the right paddle
        *dx = (-1) * (*dx);
    }
...
void drawPaddleAtPos(int x,int y)
{
    ...
    ST7735_DrawBitmap(123, y, paddle_2, 2, 16);
}
